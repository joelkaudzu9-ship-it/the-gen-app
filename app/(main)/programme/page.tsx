          ...
          {days.map((day) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl font-medium whitespace-nowrap ${isActive ? 'shadow-gold' : 'border'}`}
                style={{
                  padding: '8px 16px',
          ...
          sessions.map((session, index) => {
            ...
            return (
              <AnimatedSection key={session.id} delay={index * 0.05}>
                <GlassCard
                  dark
                  className={`${isCurrent ? 'border-l-4 border-brand-gold shadow-gold' : ''} ${isPast ? 'opacity-60' : ''}`}
                  hover={!isPast}
                >
                  <div className="flex items-start gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2" style={{ flexWrap: 'wrap' }}>
                        ...
                        {isCurrent && (
                          <span
                            className="bg-brand-gold/20 text-brand-gold rounded-full animate-pulse"
                      ...
                      <div className="flex gap-3 mt-2 text-sm" style={{ flexWrap: 'wrap' }}>
                        {session.location && (
                      ...
                      {session.speaker && (
                        <p className="text-sm mt-2 flex items-center gap-1" style={{ color: 'rgba(212,175,55,0.7)' }}>
